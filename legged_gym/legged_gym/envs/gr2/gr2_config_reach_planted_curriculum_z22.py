...
class GR2ReachPlantedCurriculumZ22Cfg(GR2ReachPlantedCurriculumZ17Cfg):
    """Posture-first recovery branch: keep compact motion before extending low reach."""

    class reach(GR2ReachPlantedCurriculumZ17Cfg.reach):
        target_z_range = [-0.45, 0.239]
        target_y_abs_range = [0.03, 0.14]
        target_x_range = [0.18, 0.30]
        ...
        target_edge_high_z_weight = 0.18
    ...
    class rewards(GR2ReachPlantedCurriculumZ17Cfg.rewards):
        ...
        pickup_feet_width_increase_target = 0.008
        arm_leg_clearance_margin = 0.24

        class scales(GR2ReachPlantedCurriculumZ17Cfg.rewards.scales):
            reach_target_pos = 14.00
            reach_target_success = 6.00
            reach_target_stable = 4.00

            low_target_forward_torso_pitch = 6.50
            low_target_active_shoulder_drop = 1.80

            low_target_knee_flexion = -0.15
            low_target_excess_knee_flexion = -11.00
            ...
            low_target_stance_width = -5.80
            low_target_leg_asymmetry = -8.00
            low_target_torso_lateral_tilt = -9.00
            low_target_yaw_twist = -11.00

            feet_xy_displacement = -7.20
            feet_speed_xy = -1.60
            inactive_arm_still = -0.75
            arm_leg_clearance = -8.50
            collision = -1.60


class GR2ReachPlantedCurriculumZ22CfgPPO(
    GR2ReachPlantedCurriculumZ17CfgPPO,
    ...
    class runner(GR2ReachPlantedCurriculumZ17CfgPPO.runner):
        ...
        run_name = "planted_curriculum_z22_posture_first_from_z17_15599"
        max_iterations = 200
        save_interval = 25
    ...
    class policy(GR2ReachPlantedCurriculumZ17CfgPPO.policy):
        init_noise_std = [0.010] * GR2ReachPlantedCurriculumZ22Cfg.env.num_actions
